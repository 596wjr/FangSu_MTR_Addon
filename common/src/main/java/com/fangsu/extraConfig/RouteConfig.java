...
/**
 * 线路选择配置项：按键固定为打开线路选择界面。
 * 存储选中线路的 id（Long）。
 */
public class RouteConfig extends ConfigEntry<Long> {
    ...
    public static RouteConfig fromLocal(
            LocalComponent title,
            ConfigSpec spec,
            Supplier<Long> getter,
            Consumer<Long> setter
    ) {
        return new RouteConfig(title.getRaw(), spec, getter, setter);
    }

    public RouteConfig setButtonText(Component text) {
