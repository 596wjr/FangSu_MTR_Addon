...
public final class SisDrawManager {
    ...
    public static BaseSisDrawing createDrawing(String key) {
        ...
        if (className != null) {
            try {
                ...
                if (BaseSisDrawing.class.isAssignableFrom(clazz)) {
                    @SuppressWarnings("unchecked")
                    BaseSisDrawing instance = (BaseSisDrawing) clazz.getDeclaredConstructor().newInstance();
                    drawingCache.put(key, instance);
                    return instance;
                }
            } catch (Exception e) {
                Main.LOGGER.error("Failed to reflectively create SIS drawing: {} class={}", key, className, e);
            }
        }
    ...
    private static void injectScriptDrawers() {
        try {
            ...
            Main.LOGGER.error("Failed to preload SIS draw scripts", e);
        }
    }
